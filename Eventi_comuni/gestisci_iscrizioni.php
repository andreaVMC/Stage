<?php
    ...
    if (isset($_POST['carica'])) {
        ...
        foreach ($eventiPrenotati as $evento) {
            $nome= "fattura".$evento['Id'];
            $file = $_FILES[$nome];
            $fileName = $file['name'];
            $fileTmpName = $file['tmp_name'];
            $fileDestination = 'fatture/' . $fileName;
            if($fileName!=""){
                move_uploaded_file($fileTmpName, $fileDestination);
                chmod($fileDestination, 0666);
                $stmt = $connessione->prepare("UPDATE `registro` SET `fattura_pagata` = ? WHERE `Id` = ?");
                $stmt->execute([$fileDestination,$evento['Id']]);
            }
        }
    }
?>
...
<div class="action">    
<button onclick="location.href='<?php echo 'userDashboard.php?username=' . $username; ?>'">dashboard</button></div>
<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . '?username=' . $username; ?>" method="post"
    ...
    <table border="1px solid black">
        <tr>
            ...
            <th>sede</th>
            <th>abbonamento</th>
            <th>scarica fattura</th>
            <th>carica fattura</th>
            <th>scarica ricevuta</th>
            ...
            <tr>
                ...
                <td>
                    <button onclick="scarica_fattura(
                        '<?php echo $username ?>',
                        '<?php echo getNomeComuneById($evento['Id_comune'], $connessione) ?>',
                        '<?php echo getNomeEventoById($evento['Id_evento'], $connessione) ?>',
                        '<?php echo $evento['sede_evento'] ?>',
                        '<?php echo $evento['tipo_abbonamento'] ?>'
                        )">scarica fattura
                    </button>
                </td>
                <td>
                    <input type="file" name="<?php echo "fattura".$evento['Id'] ?>" id="<?php echo "fattura".$evento['Id'] ?>" style="display:none;">
                    <button type="button" onclick="document.getElementById('<?php  echo 'fattura'.$evento['Id'] ?>').click()">carica fattura</button>
                </td>
                <td>
...
<button onclick="document.getElementById('submit_carica').click()">carica</button>
</div>
<script>
    function scarica_fattura(nomeTitolare, nomeComune, nomeEvento, indirizzoEvento, tipoAbbonamento) {
        // Contenuto del file
        var contenuto = "fattura di " + nomeTitolare + "\nconferma prenotazione dell' evento " + nomeEvento + " ,presso " + indirizzoEvento + ",\nnel comune di " + nomeComune + ", al seguito dell' abbonamento di tipo " + tipoAbbonamento + ".\nFirma titolare:____________________";
        ...
        downloadLink.download = "fattura_" + nomeTitolare + "_" + nomeEvento + "_" + tipoAbbonamento + ".txt";
    ...
    function scarica_ricevuta(nomeTitolare, nomeComune, nomeEvento, indirizzoEvento, tipoAbbonamento) {
        // Contenuto del file
        var contenuto = "ricevuta di " + nomeTitolare + "\nconferma pagmento della prenotazione per l' evento " + nomeEvento + " ,presso " + indirizzoEvento + ",\nnel comune di " + nomeComune + ", al seguito dell' abbonamento di tipo " + tipoAbbonamento + ".";
        // Creazione dell'elemento <a> per il download
        var downloadLink = document.createElement("a");
        downloadLink.href = "data:text/plain;charset=utf-8," + encodeURIComponent(contenuto);
        downloadLink.download = "ricevuta_" + nomeTitolare + "_" + nomeEvento + "_" + tipoAbbonamento + ".txt";
